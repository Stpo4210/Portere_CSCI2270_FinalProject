...
int main(){
    ...
    if(response == "n"){
        ...
        while(running){ //while the game is running
            ...
            cout<<"Given your available moves, would you like to:"<<endl<<"(w) Go forward? "<<endl<<"(s) Go backward?"<<endl<<"(a) Go left?"<<endl<<"(d) Go right?"<<endl<<"(1) Place marker?"<<endl<<"(2) Look at inventory?"<<endl<<"(q) Get help?"<<endl<<"(p) Give up?"<<endl;
            cin.clear();
            cin>>response;
            if(response == "w"){
                one->moveForward();
